...
const bindStacyBubble = () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    // Smart context: try to understand what they're asking
    const tenants = allTenants(property);
    const nameMatch = tenants.find(t => lower.includes(t.fullName.toLowerCase()));
    if (nameMatch) {
      await reply(`Found tenant: ${nameMatch.fullName}\nUnit: ${nameMatch.unit.label}\nPhone: ${nameMatch.phone || "Not on file"}\nEmail: ${nameMatch.email || "Not on file"}\nVehicles: ${nameMatch.vehicles?.length || 0} registered\n\nNeed to do something with this tenant?`);
      return;
    }

    // Default: helpful context without markdown
    await reply(`I can help with a few things from here:\n\n- Look up a plate: just type or paste the license plate\n- Find a tenant: say "find [name]" or "contact unit 2A"\n- Parking complaint: say "parking complaint" and I'll guide you\n- Maintenance: describe the issue and I'll create a ticket\n- Property summary: say "summary"\n\nWhat do you need?`);
  });
};
